...
object DateUtils {
   ...
   * @return Date
   */
  def parse(time: String, format: String): Date = {
    val formatter: DateTimeFormatter = DateTimeFormat.forPattern(format)
    formatter.parseDateTime(time).toDate
  }
   ...
   */
  def convertDateTime(time: String) = {
    formatDateTime(parseTime(time), FORMAT_DATE_TIME_SECOND)
  }
}
